...
@nb.njit(cache=True)
def lyapunov_max_single(E, m, k, alpha, beta, dt, n_sample, n_renorm, Delta0=np.array([1e-8, 0., 0., 0.]), offset=1e-300):
    P20 = np.sqrt(2.0 * m * E) 
    P10 = np.sqrt(2.0 * m * E)
    ...
    for cycle in range(n_cycles):
        for _ in range(n_renorm):
            ...
            Q = Leapfrog_step(Q, m, k, alpha, beta, dt)
            Qp = Leapfrog_step(Qp, m, k, alpha, beta, dt)
        Delta = Qp[0] - Q[0]
        d = np.linalg.norm(Delta)
        lambda_sum += np.log(np.abs(d / d0) + offset) # 累加
        Qp = Q + (Delta / d) * d0
